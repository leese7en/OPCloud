...
#define OP_FAIR			0x0100  ///< 点值为强制输入
#define OP_POOR			0x0200  ///< 计算点的来源部分为坏值
...
#define OP_ALARM_MASK	0x009E  ///< 点状态的报警掩码，7,x,x,4, 3,2,1,x
...
//#define USE_TIME_MS             ///< 从2.11版本开始，默认支持毫秒
...
*   @li 失败: -1
*/
GMacroAPI(int) op2_select(OpenPlant op, const char *fieldlist, int num, OPObject *objects, int *errors);
...
* @return 结果集尺寸
*/
GMacroAPI(int) op2_alarm_num_rows(OPResult result);
...
*   @li 其它: 出错
*/
GMacroAPI(int) op2_alarm_fetch_value(OPResult result, AlarmVal *aval);
...
* @return 本函数没有返回值
*/
GMacroAPI(void) op2_alarm_free_result(OPResult result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_real_alarm_byid(OpenPlant op, int num, int *id, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_real_alarm_byname(OpenPlant op, OPGroup gh, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_real_child_alarm_byid(OpenPlant op, int id, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_real_child_alarm_byname(OpenPlant op, const char *obj_name, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_hist_alarm_byid(OpenPlant op, int num, int *id, OPTime begin_tm, OPTime end_tm, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_hist_alarm_byname(OpenPlant op, OPGroup gh, OPTime begin_tm, OPTime end_tm, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_hist_child_alarm_byid(OpenPlant op, int id, OPTime begin_tm, OPTime end_tm, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_get_hist_child_alarm_byname(OpenPlant op, const char *obj_name, OPTime begin_tm, OPTime end_tm, OPResult *result);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_alarm_ack_byid(OpenPlant op, int num, int *id, int *errors);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_alarm_ack_byname(OpenPlant op, OPGroup gh, int *errors);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_alarm_inhibit_byid(OpenPlant op, int num, int *id, int inhibit, int *errors);
...
*   @li 失败: 错误码
*/
GMacroAPI(int) op2_alarm_inhibit_byname(OpenPlant op, OPGroup gh, int inhibit, int *errors);
