...
(function($) {
    ...
    function Scroller(elem, settings) {
        ...
        function read() {
            that.temp = ((input && that.val !== null && that.val != elm.html()) || that.values === null) ? s.parseValue(elm.html() || '', that) : that.values.slice(0);
            that.setValue(true);
        }
        ...
        function position(check) {
            ...
            ww = $(window).width() * DPR;
            rwh = $(window).height() * DPR;
            var windowWidth = ww;
            var windowHeight = rwh;
            ...
            $('.dw-persp', dw).height(0).height(t + mh > windowHeight ? t + mh : windowHeight);
            $('.dw-persp').css('width', windowWidth);
            // Scroll needed
        ...
        that.setValue = function(sc, fill, time, temp) {
            ...
            var tempVal = elm.html();
            if (tempVal != '开始时间' && tempVal != '结束时间' && !fill) {
                var ymdhms = tempVal.split(' ');
                var ymd = ymdhms[0].split('-');
                var hms = ymdhms[1].split(':');
                var tempY = ymd[0];
                var tempMM = ymd[1];
                tempMM = parseInt(tempMM) - 1;
                tempMM = tempMM < 10 ? '0' + tempMM : tempMM;
                var tempD = ymd[2];
                var tempH = hms[0];
                var tempM = hms[1];
                var tempS = hms[2];
                that.temp = [tempY, tempMM, tempD, tempH, tempM, tempS];
                v = tempVal;
            } else {
