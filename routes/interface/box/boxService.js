                ...
                async.waterfall(
                    [
                                ...
                                conn.query(sql, function (error, rows) {
                                    if (error) {
                                        ...
                                        if (rows.length < 1) {
                                            ...
                                            var sqlPoint = sqlQuery.remove().from('sys_point').where({point_id: ID}).build();
                                            ...
                                            conn.query(sql, function (error, rows) {
                                                if (error) {
                                                    ...
                                                    logger.debug('删除采集下面MTree测点sql:', sqlPoint);
                                                    conn.query(sqlPoint, function (error, rows) {
                                                        if (error) {
                                                            logger.error('删除采集MTree测点信息错误：', error);
                                                            message.Status = false;
                                                            message.message = '删除采集MTree测点信息失败';
                                                            message.Data = null;
                                                            callback(error, message);
                                                        } else {
                                                            callback(null, updateDas, dasServices);
                        ...
                        function (insertDas, updateDas, deleteDas, callback) {
                            var sql = 'select id as id,URI,DOMAIN_ID as domainId,company_id as companyId from sys_mtree where MTREE_SOURCE =3 and  name ="' + devCode + '"';
                            ...
                            conn.query(sql, function (error, rows) {
                                if (error) {
                                    ...
                                    if (rows.length == 1) {
                                        callback(null, insertDas, updateDas, deleteDas, rows[0]);
                                    } else {
                                        message.Status = false;
                        ...
                        function (insertDas, updateDas, deleteDas, mtree, callback) {
                            if (insertDas.length > 0) {
                                ...
                                var sqlConf = 'insert into box_das_conf (dev_code,das_name,keyId,Descs,Tips,Scope,defaults,TextType,TextData,CREATE_DATE) values '
                                var sqlMTree = 'insert into sys_mtree (DOMAIN_ID,URI,NAME,LAYER,DESCRIPTION,COMPANY_ID,MTREE_SOURCE,CREATE_DATE) values '
                                var date = '"' + Utils.dateFormat(new Date().getTime(), Utils.yyyyMMddhhmmss) + '"';
                                var size = insertDas.length;
                                for (var i = 0; i < size; i++) {
                                    var v = insertDas[i];
                                    ...
                                    for (var j = 0; j < confSize; j++) {
                                        var conf = dasConf[j];
                                        var value = '("' + devCode + '","' + v.Name + '","' + conf.Key + '","' + conf.Name + '","' + conf.Ed + '","' + conf.ExScope + '","' + conf.Value + '","' + conf.TextType + '","' + conf.TextData + '",' + date + ')'
                                        sqlConf += value;
                                        if (dasConf[j + 1]) {
                                            sqlConf += ',';
                                        }
                                    }
                                    var mtreeValue = '(' + mtree.domainId + ',"' + mtree.URI + '/' + p.name + '","' + p.name + '",0,"同步box创建采集",' + mtree.companyId + ',4,' + date + ')';
                                    sqlMTree += mtreeValue;
                                    if (insertDas[i + 1]) {
                                        sql += ',';
                                        sqlConf += ',';
                                ...
                                conn.query(sql, function (error, rows) {
                                    if (error) {
                                        ...
                                        logger.debug('同步添加盒子采集配置信息sql:', sqlConf);
                                        conn.query(sqlConf, function (error, rows) {
                                            if (error) {
                                                ...
                                                logger.debug('同步添加盒子MTree信息sql:', sqlMTree);
                                                conn.query(sqlMTree, function (error, rows) {
                                                    if (error) {
                                                        logger.error('同步添加盒子MTree错误：' + error);
                                                        message.Status = false;
                                                        message.message = '同步添加盒子MTree出错';
                                                        message.Data = null;
                                                        callback(error, message);
                                                    } else {
                                                        callback(null, updateDas, deleteDas, mtree);
                                                    }
                                                });
                                            }
                                        });
                        ...
                        function (updateDas, deleteDas, mtree, callback) {
                            if (updateDas.length > 0) {
                                var size = updateDas.length;
                                ...
                                conn.query(sqlDas, function (err, rows) {
                                    if (err) {
                        ...
                        function (deleteDas, mtree, callback) {
                            if (deleteDas.length > 0) {
                                var size = deleteDas.length;
                                var dasNames = [];
                                var mtreeURI = [];
                                for (var i = 0; i < size; i++) {
                                    ...
                                    mtreeURI.push(mtree.URI + '/' + deleteDas[i].name);
                                }
                                ...
                                var sqlMtree = sqlQuery.remove().from('sys_mtree').where({
                                    MTREE_SOURCE: 4,
                                    URI: mtreeURI
                                }).build();
                                    ...
                                    if (err) {
                                        ...
                                        conn.query(sqlConf, function (error, rows) {
                                            if (err) {
                                                ...
                                                logger.debug('同步删除盒子MTree信息sql:', sqlMtree);
                                                conn.query(sqlMtree, function (error, rows) {
                                                    if (err) {
                                                        logger.error('同步删除MTree信息错误：', error);
                                                        message.Status = false;
                                                        message.message = '同步删除MTree出错';
                                                        message.Data = null;
                                                        callback(error, message);
                                                    } else {
                                                        callback(null, mtree);
                        ...
                        function (mtree, callback) {
                            ...
                            conn.query(sql, function (error, rows) {
                                if (error) {
                                    ...
                                    callback(null, rows, mtree);
                                }
                            });
                        },
                        function (dasPoints, mtree, callback) {
                            var insertPoint = [];
                            var deletePoint = [];
                            var updatePoint = [];
                            ...
                            callback(null, insertPoint, deletePoint, updatePoint, dasPoints, mtree);
                        },
                        function (insertPoint, deletePoint, updatePoint, dasPoints, mtree, callback) {
                            if (insertPoint.length > 0) {
                                ...
                                boxSocket.sendMsg(EDM_ADD_POINT, devCode, COMMAND_FLAG, JSON.stringify(objArray), function (error, data) {
                                    if (error) {
                                        ...
                                        if (data != null) {
                                            if (data.charCodeAt() == OPERATE_SUCCESS) {
                                                callback(null, deletePoint, updatePoint, dasPoints, mtree);
                                            } else {
                                                message.Status = false;
                        ...
                        function (deletePoint, updatePoint, dasPoints, mtree, callback) {
                            if (deletePoint.length > 0) {
                                ...
                                boxSocket.sendMsg(EDM_DEL_POINT, devCode, COMMAND_FLAG, JSON.stringify(objArray), function (error, data) {
                                    if (error) {
                                        ...
                                        if (data != null) {
                                            if (data.charCodeAt() == OPERATE_SUCCESS) {
                                                callback(null, updatePoint, dasPoints, mtree);
                                            } else {
                                                message.Status = false;
                        ...
                        function (updatePoint, dasPoints, mtree, callback) {
                        ...
                        function (dasPoints, mtree, callback) {
                            boxSocket.sendMsg(EDM_ALL_POINT, devCode, COMMAND_FLAG, null, function (error, data) {
                                if (error) {
                                    ...
                                    callback(null, JSON.parse(data), dasPoints, mtree);
                                }
                            });
                        },
                        function (boxPoints, cloudPoints, mtree, callback) {
                            var insertPoint = [];
                            var updatePoint = [];
                            var deletePoint = [];
                            ...
                            callback(null, insertPoint, updatePoint, deletePoint, mtree);
                        },
                        function (insertPoint, updatePoint, deletePoint, mtree, callback) {
                            if (insertPoint.length > 0) {
                                var sql = 'SELECT ID,URI,name from sys_mtree where  URI LIKE "' + mtree.URI + '/%"';
                                logger.debug('获取下属采集信息sql:', sql);
                                query(sql, function (error, rows) {
                                    if (error) {
                                        logger.error('获取采集信息错误:', error);
                                        message.Status = false;
                                        message.message = '获取采集信息配置';
                                        message.Data = null;
                                        callback(new Error('获取采集信息配置错误'), message);
                                    } else {
                                        if (rows.length < 1) {
                                            message.Status = false;
                                            message.message = '没有采集存在';
                                            message.Data = null;
                                            callback(new Error('没有采集存在'), message);
                                        } else {
                                            var size = rows.length;
                                            var mtrees = {};
                                            for (var i = 0; i < size; i++) {
                                            ...
                                            callback(null, insertPoint, updatePoint, deletePoint, mtree, mtrees);
                                        }

                                    }
                                });
                            } else {
                                callback(null, insertPoint, updatePoint, deletePoint, mtree, null);
                            }
                        },
                        function (insertPoint, updatePoint, deletePoint, mtree, mtrees, callback) {
                            if (insertPoint.length > 0) {
                                var cols = [];
                                var rows = [];
                                ...
                                for (var i = 0; i < length; i++) {
                                    var point = insertPoint[i];
                                    var row = [];
                                    var Pn = point.Pn.replace(point.Sn + '_', '');
                                    point.Pn = Pn;
                                    var pointURI = mtree.URI + '/' + point.Sn + '/' + point.Pn;
                                    row.push(configUtils.getPointIDNext());
                                    var uuid = opPool.makeUUID(pointURI).toString();
                                    point.Gn = 'W3.NODE.' + uuid;
                                    row.push(point.Gn);
                                    row.push(point.Pn);
                                    uuid = '0x' + uuid;
                                    row.push(uuid);
                                    row.push(mtree.domainId);
                                    row.push(mtree.companyId);
                                ...
                                opPool.insert('Point', rows, cols, function (error, rrs) {
                                    if (error != 0 && error.code) {
                                        ...
                                        if (points.length < 1) {
                                            ...
                                            var sqlPoints = 'insert into sys_point (`ID`,`POINT_ID`, `UUID`, `URI`, `MTREE_ID`, `POINT_NAME`, `UNIT`, `POINT_TYPE`,`DESCRIPTION`,`CREATE_DATE`,`ENTERPRISE_ID`,`DOMAIN_ID`) values ';
                                            var date = '"' + Utils.dateFormat(new Date().getTime(), Utils.yyyyMMddhhmmss) + '"';
                                            ...
                                            for (var i = 0; i < pointSize; i++) {
                                                ...
                                                sqlValue += '\'' + p.Ud + '\',';
                                                sqlValue += '\'' + p.URI + '\',';
                                                sqlValue += mtrees[p.Sn] ? mtrees[p.Sn].ID : 0 + ',';
                                                sqlValue += '\'' + p.Pn + '\',';
                                                sqlValue += '"",';
                                                sqlValue += p.RT + ',';
                                                sqlValue += '\'' + p.Ed + '\',';
                                                sqlValue += date + ',';
                                                sqlValue += mtree.companyId + ',';
                                                sqlValue += mtree.domainId;
                                                sqlValue += ')';
                                                sqlPoints += sqlValue;
                                                if (points[i + 1]) {
                                                    sql += ',';
                                            ...
                                            conn.query(sql, function (err, rows) {
                                                if (err) {
                                                    ...
                                                    logger.debug('同步添加MTree测点信息sql:' + sqlPoints);
                                                    conn.query(sqlPoints, function (err, rows) {
                                                        if (err) {
                                                            logger.error('同步添加MTree测点信息错误：' + err);
                                                            message.Status = false;
                                                            message.message = '同步添加MTree测点出错';
                                                            message.Data = null;
                                                            callback(new Error('同步添加MTree测点出错'), message);
                                                        } else {
                                                            callback(null, updatePoint, deletePoint, mtree);
                                                        }
                                                    });
                                                }
                                            });
                        ...
                        function (updatePoint, deletePoint, mtree, callback) {
                            if (updatePoint.length > 0) {
                                var cols = [];
                                var rows = [];
                                cols.push(["GN", OPAPI.TYPE.STRING]);
                                ...
                                opPool.update('Point', rows, cols, function (error, rrs) {
                                    if (error && error != 0 && error.code) {
                                        ...
                                        var sqlPoint = 'update sys_point set description = case URI ';
                                        var gns = '';
                                        var uris = '';
                                        var anSql = '';
                                        var ptSql = '';
                                        ...
                                        sqlPoint += pointSql + ' end ,update_date=' + date + ' where URI in (' + uris + ')';
                                        ...
                                        conn.query(sql, function (error, rows) {
                                            if (error) {
                                                ...
                                                logger.debug('同步更新MTree测点信息sql:' + sqlPoint);
                                                conn.query(sqlPoint, function (error, rows) {
                                                    if (error) {
                                                        logger.error('同步更新MTree测点信息错误：' + error);
                                                        message.Status = false;
                                                        message.message = '同步更新MTree测点出错';
                                                        message.Data = null;
                                                        callback(error, message);
                                                    } else {
                                                        callback(null, deletePoint, mtree);
                                                    }
                                                });
                                            }
                                        });
                        ...
                        function (deletePoint, mtree, callback) {
                            if (deletePoint.length > 0) {
                        ...
                        function (gns, uds, callback) {
                            var sql = sqlQuery.remove().from('box_das_point').where({
                                dev_code: devCode,
                                gn: gns
                            }).build();
                            var sqlPoint = sqlQuery.remove().from('sys_point').where({
                                UUID: uds
                            }).build();
                            ...
                            conn.query(sql, function (error, rows) {
                                if (error) {
                                    ...
                                    logger.debug('同步删除MTree测点信息sql:' + sqlPoint);
                                    conn.query(sqlPoint, function (error, rows) {
                                        if (error) {
                                            logger.error('同步删除MTree测点信息错误：' + error);
                                            message.Status = false;
                                            message.message = '同步删除MTree测点出错';
                                            message.Data = null;
                                            callback(error, message);
                                        } else {
                                            message.Status = true;
