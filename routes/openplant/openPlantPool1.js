...
//
// var sql = 'select ID,GN,RT from V_Point where ID in (116237)';
...
var sql = 'select ID,TN,ED,RT,TM,DS,AV from realtime where KR in (4) order by ID limit 0,10';
...
// var sql = 'select ID,RT,TN,PN,ED from point where KR in (72) and ID in (116299,116300)';
// opPool.query(sql, function (error, rows, columns) {
//     logger.error(error);
//     logger.error(rows);
//     logger.error(columns);
...
// console.log(ud);
// var sql = 'select kr,sum(KR) as count from Point group by KR';
// // // // var UD = '0x'sum+ud;
...
// var agentHandler = opPool.subscribe([1024, 1025]);
...
// }
// setTimeout(function() {
//     var agentHandler1 = opPool.subscribe([1024]);
//     opPool.startSub(test_event_callback);
// }, 5000);
